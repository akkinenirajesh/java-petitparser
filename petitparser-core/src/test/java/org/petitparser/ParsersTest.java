...
public class ParsersTest {
	...
	public void testFlatten1() {
		Parser parser = CharacterParser.digit().repeat(2, RepeatingParser.UNBOUNDED).flatten();
	...
	public void testFlatten2() {
		Parser parser = CharacterParser.digit().repeat(2, RepeatingParser.UNBOUNDED).flatten("gimme a number");
	...
	@Test
	public void testMap() {
		Parser parser = CharacterParser.digit().map((Function<Character, Integer>) Character::getNumericValue);
	...
	public void testRange() {
		Parser parser = CharacterParser.digit().trim().plus();
		Result result = parser.parse("11\n\r1");
	...
	public void testRepeatGreedyUnbounded() {
		...
		builderDigit.append('1');
		Parser parser = CharacterParser.word().repeatGreedy(CharacterParser.digit(), 2, RepeatingParser.UNBOUNDED);
	...
	public void testRepeatLazyUnbounded() {
		...
		builder.append("1111");
		Parser parser = CharacterParser.word().repeatLazy(CharacterParser.digit(), 2, RepeatingParser.UNBOUNDED);
	...
	public void testContinuationSuccessful() {
		Parser parser = CharacterParser.digit()
				.callCC((continuation, context) -> context.success("Always succeed", 0, 0));
		assertTrue(parser.parse("1").isSuccess());
		assertTrue(parser.parse("a").isSuccess());
	}
	...
	public void testContinuationFailing() {
		Parser parser = CharacterParser.digit().callCC((continuation, context) -> context.failure("Always fail"));
		assertFalse(parser.parse("1").isSuccess());
		assertFalse(parser.parse("a").isSuccess());
	}
}
