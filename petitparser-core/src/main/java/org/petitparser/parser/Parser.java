...
public abstract class Parser {
	 ...
	 * resulting context, which is either a {@link org.petitparser.context.Success}
	 ...
	 * getting its speed advantage by avoiding any unnecessary memory allocations.
	 *
	 * <p>
	 ...
	 * current {@code position} in that buffer. It returns a new (positive) position
	 * in case of a successful parse, or `-1` in case of a failure.
	 *
	 * <p>
	...
	public <T> List<T> matches(String input) {
		...
		and().mapWithSideEffects(list::add).seq(any()).or(any()).star().fastParseOn(new MultiLineStringBuffer(input),
				0);
		return (List<T>) list;
	}
	 ...
	 * parser returns the result of the receiver, or {@code null} if not applicable.
	 */
	...
	/**
	 * Returns new parser that accepts the receiver, if possible. The returned value
	 * can be provided as {@code otherwise}.
	 */
	...
	/**
	 * Returns a parser that accepts the receiver zero or more times. The resulting
	 * parser returns a list of the parse results of the receiver.
	 *
	 * <p>
	 * This is a greedy and blind implementation that tries to consume as much input
	 * as possible and that does not consider what comes afterwards.
	 */
	public Parser star() {
	...
	/**
	 * Returns a parser that parses the receiver zero or more times until it reaches
	 * a {@code limit}.
	 *
	 * <p>
	 ...
	 * {@link Parser#repeat(int, int)} operator. The {@code limit} is not consumed.
	 */
	 ...
	 * resulting parser returns a list of the parse result of the receiver followed
	 * by the parse result of {@code others}.
	 *
	 * <p>
	 * Calling this method on an existing sequence code not nest this sequence into
	 * a new one, but instead augments the existing sequence with {@code
	 * others}.
	 */
	...
	/**
	 * Returns a parser (logical and-predicate) that succeeds whenever the receiver
	 * does, but never consumes input.
	 */
	public Parser and() {
	...
	/**
	 * Returns a parser (logical not-predicate) that succeeds whenever the receiver
	 * fails, but never consumes input.
	 */
	public Parser not() {
	...
	/**
	 * Returns a parser that discards the result of the receiver, and returns a
	 * sub-string of the consumed range in the string/list being parsed. Report the
	 * provided {@code message} in case of an error.
	 */
	 ...
	 * value of the receiver {@link Token#getValue()}, as well as the consumed input
	 * {@link Token#getInput()} from {@link Token#getStart()} to
	 * {@link Token#getStop()} of the input being parsed.
	 */
	public Parser token() {
	...
	/**
	 * Returns a parser that points to the receiver, but can be changed to point to
	 * something else at a later point in time.
	 */
	 ...
	 * @param function production action without side-effects.
	 */
	public <B> Parser mapResult(Function<Result, B> function) {
	 ...
	 * permuted elements at {@code indexes} of a list. Negative indexes can be used
	 * to access the elements from the back of the list.
	 */
	...
	/**
	 * Returns a new parser that parses the receiver one or more times, separated by
	 * a {@code separator}.
	 */
	 ...
	 * a {@code separator}.
	 */
	public Parser separatedBy2(Parser separator) {
		return new SequenceParser(this, new SequenceParser(separator, this).pick(1).star())
				.map((List<List<Object>> input) -> {
					List<Object> result = new ArrayList<>();
					result.add(input.get(0));
					input.get(1).forEach(result::add);
					return result;
				});
	}
	 ...
	 * and possibly ended by a {@code separator}."
	 */
	public Parser delimitedBy2(Parser separator) {
		return separatedBy2(separator).seq(separator.optional()).pick(0);
	}
	...
	protected boolean isEqualTo(Parser other, Set<Parser> seen) {
		...
		seen.add(this);
		return Objects.equals(getClass(), other.getClass()) && hasEqualProperties(other)
