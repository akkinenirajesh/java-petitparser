...
public class TokenParser extends DelegateParser {
	...
	public Result parseOn(Context context) {
		...
		if (result.isSuccess()) {
			Token token = new Token(context.getBuffer().getBuffer(), context.getPosition(), result.getPosition(), result.get());
			return result.success(token, context.getPosition(), result.getPosition());
		} else {
			return result;
		}
	}
