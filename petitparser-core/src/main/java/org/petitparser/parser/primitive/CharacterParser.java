...
public class CharacterParser extends Parser {
	 ...
	 *
	 * <p>
	 * Characters match themselves. A dash {@code -} between two characters matches
	 * the range of those characters. A caret {@code ^} at the beginning negates the
	 * pattern.
	 */
	...
	public Result parseOn(Context context) {
		...
		if (position < buffer.length()) {
			...
			if (matcher.test(result)) {
				return context.success(result, position, position + 1);
			}
		}
	...
	protected boolean hasEqualProperties(Parser other) {
		return super.hasEqualProperties(other) && Objects.equals(matcher, ((CharacterParser) other).matcher)
