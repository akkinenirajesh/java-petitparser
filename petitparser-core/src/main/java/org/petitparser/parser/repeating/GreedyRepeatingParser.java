...
public class GreedyRepeatingParser extends LimitedRepeatingParser {
	...
	public Result parseOn(Context context) {
		...
		while (true) {
			...
			Result limiter = limit.parseOn(lastContext);
			if (limiter.isSuccess()) {
				return lastContext.success(elements, context.getPosition(),
						lastContext.getPosition());
			}
			if (elements.isEmpty()) {
				return limiter;
			}
	...
	public int fastParseOn(MultiLineStringBuffer buffer, int position) {
		...
		while (true) {
			int limiter = limit.fastParseOn(buffer, positions.get(positions.size() - 1));
			if (limiter >= 0) {
